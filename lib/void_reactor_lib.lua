...
function reactorLib.initData()
    reactorComponents = {}
    for address, _ in pairs(component.list("htc_reactors_nuclear_reactor")) do
...
function reactorLib.powerControl(currentLiquidCount, liquidSettings)
    if currentLiquidCount < liquidSettings.lapisMinimum and reactorsEnabled == ReactorState.WORKING then
        --Выключение при малом количестве лазурита
        --gpu.set(xr - 26, 4, 'Мало лазурита. Выключение')
        reactorLib.stopAll()
        --gpu.set(xr - 26, 4, 'Реакторы выключены        ')
    elseif currentLiquidCount >= liquidSettings.lapisRecomended and reactorsEnabled == ReactorState.STOPPED then
